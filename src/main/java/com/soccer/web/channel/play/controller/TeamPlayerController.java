...
//	}
	...
	public String selectChannelPlayFormation(	@PathVariable int channelIdx,
		...
		try {
			...
			// 채널에 엮인 팀이 아닌 경우를 위한 메서드
			List<TeamVO> tmpTeamVOList = channelPlayService.selectNotLinkedTeamList(channelPlayIdx);
			if (tmpTeamVOList.size() != 0) {
				if (teamVOList.size() == 0) {
					for (TeamVO tmpItem : tmpTeamVOList) {
						teamVOList.add(tmpItem);
					}
				} else if (teamVOList.size() == 1) {
					String tmpTeamType = teamVOList.get(0).getTeamType();
					for (TeamVO tmpItem : tmpTeamVOList) {
						if (!tmpItem.getTeamType().equals(tmpTeamType)) {
							teamVOList.add(tmpItem);
						}
					}
				}
			...
			System.out.println("teamVOList : " + teamVOList.toString());
			...
			for (TeamVO item : teamVOList) {
				if (item.getTeamType().equals("H")) {
					homeTeam = item;
				} else if(item.getTeamType().equals("A")){
					awayTeam = item;
				}
			}
			...
			System.out.println("homeFormation : " + homeFormation);
			...
			System.out.println("homeTeamPlayerVOList : " + homeTeamPlayerVOList.toString());
			...
			if (homeFormation != null && !homeFormation.equals("")) {
			...
			if (awayFormation != null && !awayFormation.equals("")) {
			...
			TeamPlayerVO homeGoalkeeperVO = new TeamPlayerVO();
			if (homeTeamPlayerVOList.size() != 0) {
				homeGoalkeeperVO = homeTeamPlayerVOList.get(0);
			}
			System.out.println("homeGoalkeeperVO : " + homeGoalkeeperVO.toString());
			...
			TeamPlayerVO awayGoalkeeperVO = new TeamPlayerVO();
			if (awayTeamPlayerVOList.size() != 0) {
				awayGoalkeeperVO = awayTeamPlayerVOList.get(0);
			}
...
//		return "test";
	...
	public String selectChannelPlayFormationDetail(	@PathVariable int channelIdx,
		...
		try {
			ChannelPlayVO channelPlayVO = channelPlayService.selectChannelPlayDetail(channelPlayIdx);
			System.out.println("channelPlayVO : " + channelPlayVO.toString());
			String formation = "";
			...
			TeamPlayerVO goalkeeperVO = new TeamPlayerVO();
			if (teamPlayerVOList.size() != 0) {
				goalkeeperVO = teamPlayerVOList.get(0);
			}
