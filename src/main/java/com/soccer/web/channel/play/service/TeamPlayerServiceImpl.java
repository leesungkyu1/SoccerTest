...
public class TeamPlayerServiceImpl implements TeamPlayerService{
	...
	public List<TeamPlayerVO> selectHomeAwayTeamPlayerList(HashMap<String, String> teamInfoMap) throws Exception {
		List<TeamPlayerVO> teamPlayerList = teamPlayerMapper.selectHomeAwayTeamPlayerList(teamInfoMap);
		System.out.println("수정 전 teamPlayerList : " + teamPlayerList.toString());
		List<TeamPlayerVO> notLinkedTeamPlayerList = teamPlayerMapper.selectHomeAwayTeamPlayerNotLinkedList(teamInfoMap);
		System.out.println("notLinkedTeamPlayerList : " + notLinkedTeamPlayerList.toString());

		// 회원이 아닌 선수들을 불러오기 위한 메서드..
		if (teamPlayerList.size() == 0) {
			teamPlayerList = notLinkedTeamPlayerList;
		} else if (teamPlayerList.size() != notLinkedTeamPlayerList.size()) {
			for (TeamPlayerVO item : notLinkedTeamPlayerList) {
				boolean check = false;
				int teamPlayerIdx = item.getTeamPlayerIdx();
				int teamPlayerFormationNumber = item.getTeamPlayerFormationNumber();
				for (int i = 0; i < teamPlayerList.size(); i++) {
					TeamPlayerVO tmpVO = teamPlayerList.get(i);
					if (tmpVO.getTeamPlayerIdx() == teamPlayerIdx) {
						check = true;
						break;
					}
				}
				if (!check) {
					if (teamPlayerFormationNumber == 0) {
						teamPlayerList.add(item);
					} else {
						for (int i = 0; i < teamPlayerList.size(); i++) {
							Integer tmpFormationNumber = teamPlayerList.get(i).getTeamPlayerFormationNumber();
							if (tmpFormationNumber == 0) {
								continue;
							}
							if (tmpFormationNumber > teamPlayerFormationNumber) {
								teamPlayerList.add(i-1 ,item);
							}
						}
					}
				}
		...
		System.out.println("수정 후 teamPlayerList : " + teamPlayerList.toString());
